...
export default async function handler(req, res) {
  try {
    ...
    if (!body || typeof body !== 'object' || !body.skill || typeof body.skill !== 'object') {
      return sendJson(res, 400, { ok: false, error: 'skill 데이터가 필요합니다.' });
    }
    ...
    if (!webhookJson || webhookJson.ok !== true) {
      return sendJson(res, 422, { ok: false, error: 'Apps Script 등록 실패: ' + (webhookJson?.error || '알 수 없는 오류') });
    }
    ...
    return sendJson(res, 200, {
      ...
      owner: webhookJson.owner,
      mode: webhookJson.mode,
    });
  } catch (e) {
