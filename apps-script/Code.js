...
function commandListSummary() {
  return [
    "━━━━━━━━━━━━━━━━━━━━━━━━━━",
    "     APORIA 명령어 목록",
    "━━━━━━━━━━━━━━━━━━━━━━━━━━",
    "[캐릭터]    !내정보 / !캐릭터신청·승인·반려 / !수정 / !성장 / !경험치",
    "[판정·액션] !판정 / !<스탯> / !액션 / !<액션명> / !설득·기만·협박",
    "[능력]      !화력·방호·치유·재생·간섭·강화 <랭크>",
    "[스킬]      !스킬신청·승인·반려·목록 / !스킬 <스킬명>",
    "[공용스킬]  !공용스킬목록 / !공용스킬 [별명] <스킬명>",
    "[상태·스택] !상태부여·해제·목록 / !스택 / !상태템플릿목록·보기·부여",
    "[관계·일상] !관계등록·수정·목록 / !일상점사용·회복 / !침식",
    "[전투]      !피해 / !대응 방어·회피·맞대응·무대응",
    "[에너미]    !에너미생성·삭제·공격·대응·스킬·템플릿…",
    "[패시브]    !패시브목록 / !패시브등록",
    "[닉네임]    !닉네임목록·추가·삭제",
    "[인벤·장비] !인벤토리 / !장비 / !아이템지급·삭제·사용 / !장비착용·해제",
    "[경제]      !은화 / !교환 / !퀵슬롯1~3 / !<아이템명>",
    "[세션]      !fin [메시지/별명…] [--keep-status/--keep-stack/--no-clear]",
    "━━━━━━━━━━━━━━━━━━━━━━━━━━",
    "!명령어목록 상세 로 전체 인수 형식 확인  |  <> 필수  [] 선택"
  ].join("\n");
}

function commandListCommand() {
  var summary = commandListSummary();
  var detail = [
    "[캐릭터]",
    "  !내정보 / !캐릭터신청",
    "  !캐릭터승인 <신청번호> / !캐릭터반려 <신청번호> <사유>",
    "  !수정 <별명> <항목> <값> / !성장 <별명> <항목>",
    "  !경험치 <별명1> [별명2…] <경험치량>",
    "",
    "[판정·액션]",
    "  !판정 <스탯> [난이도] [보정] / !<스탯> [난이도] [보정]",
    "  !액션 <액션명> [난이도] [보정] / !설득·기만·협박",
    "",
    "[능력]  !화력·방호·치유·재생·간섭·강화 <랭크>",
    "",
    "[스킬]",
    "  !스킬신청 / !스킬승인·반려 <신청번호> / !스킬목록",
    "  !스킬 <스킬명>",
    "  !공용스킬목록 [별명] / !공용스킬 [별명] <스킬명> [대상:대상명] [보정]",
    "",
    "[상태·스택]",
    "  !상태부여 <대상> <상태명> …옵션 / !상태해제 <대상> <상태명>",
    "  !스택 <대상> <스택명> <변동>",
    "  !상태템플릿목록 / !상태템플릿보기 <템플릿명>",
    "  !상태템플릿부여 <대상> <템플릿명>",
    "",
    "[관계·일상·침식]",
    "  !관계등록·수정·목록 / !일상점사용·회복 / !침식 <대상> <변동>",
    "",
    "[전투]",
    "  !피해 <별명> <수치>",
    "  !대응 방어|회피 [보정]",
    "  !대응 맞대응 <액션|화력 랭크|스킬 스킬명> [보정]",
    "  !대응 무대응",
    "",
    "[에너미]",
    "  !에너미목록·정보·생성·불러오기·삭제",
    "  !에너미피해·회복 <별명> <수치>",
    "  !에너미공격·대응·스킬 <별명> …",
    "  !에너미스킬등록·삭제 / !에너미템플릿등록·삭제",
    "",
    "[패시브]  !패시브목록 [별명/에너미ID] / !패시브등록",
    "",
    "[닉네임]  !닉네임목록·추가·삭제 <별명> [닉네임…]",
    "",
    "[인벤·장비]",
    "  !인벤토리·장비 [별명]",
    "  !아이템지급 <별명> <아이템명> [수량] / !아이템삭제 <id>",
    "  !장비착용·해제 [별명] <아이템명|슬롯>",
    "  !아이템사용 <아이템명> [대상:별명]",
    "",
    "[경제]",
    "  !은화 <별명…> <금액> / !교환 <상대> <아이템명|은화> [수량|금액]",
    "  !퀵슬롯1·2·3 [대상:별명] / !<아이템명>",
    "",
    "[세션]",
    "  !fin [메시지/별명…] [--keep-status] [--keep-stack] [--no-clear]",
    "",
    "━━━━━━━━━━━━━━━━━━━━━━━━━━",
    "<> 필수  [] 선택"
  ].join("\n");
...
function registerPortalCharacterData(payload, outputText, application) {
  ...
  if (passives.length > 0) {
    ...
    for (var j = 0; j < passives.length; j++) {
      ...
      try {
        var pOwnerType = String(p["소유타입"] || "").trim();
        var pOwnerKey  = String(p["소유키"]   || "").trim();
        if (!pOwnerType || pOwnerType === "global") pOwnerType = "character";
        if (!pOwnerKey  || pOwnerKey  === "*")      pOwnerKey  = charName;
